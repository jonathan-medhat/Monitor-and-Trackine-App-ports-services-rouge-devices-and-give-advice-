...
class AutoCybersecurityMonitor:
    ...
    def setup_gui(self):
        ...
        # NEW: custom ports field
        tk.Label(settings_frame, text="Ports:", fg="white", bg="#1a1a1a").grid(row=0, column=4, padx=5, pady=5)
        self.ports = tk.Entry(settings_frame, width=10, font=("Courier", 10), bg="#2d2d2d", fg="white")
        # example format: 22,80,443 or 1-1024
        self.ports.insert(0, "")  
        self.ports.grid(row=0, column=5, padx=5, pady=5)
        
        tk.Label(settings_frame, text="Interval (s):", fg="white", bg="#1a1a1a").grid(row=0, column=6, padx=5, pady=5)
    ...
    def scan_ports(self, auto=False):
        ...
        ports_str = self.ports.get().strip()
        if ports_str:
            # Use custom ports
            cmd = ["nmap", "-T4", "-p", ports_str, target]
        else:
            # Fast common ports
