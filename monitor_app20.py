...
class CyberMonitor:
    def __init__(self, root):
        self.root = root
        self.root.title("Cybersecurity Monitor")
    ...
    def build_ui(self):
        ...
        self.scan_type.grid(row=0, column=4, padx=10)
    ...
    def start_capture(self):
        ...
        cmd = ["tcpdump", "-i", self.interface.get(), self.filter_entry.get()]
    ...
    # ================= PING =================
    def start_ping(self):
        target = self.ping_target.get() or self.target.get()
    ...
    def generate_advice(self):
        ...
        if self.packet_count > 100:
            advice.add("📡 High traffic detected → Monitor for attacks.")

        if len(self.netdiscover_log) > 10:
            advice.add("🖧 Many devices detected → Check unauthorized devices.")

        if not advice:
            advice.add("✅ No critical security issues detected.")
