...
class CyberMonitor:
    ...
    def build_ui(self):
        ...
        ctk.CTkButton(
            netd, text="▶ NETDISCOVERY",
            fg_color="#6366f1",
            command=self.start_netdiscover
        ).grid(row=0, column=2, padx=5)

        ctk.CTkButton(
            netd, text="■ STOP NETDISCOVERY",
            fg_color="#ef4444",
    ...
    def start_netdiscover(self):
        ...
        if not router_ip:
            self.log("Netdiscover: router IP is empty")
            return

        ip_range = f"{router_ip}/24"
        cmd = ["netdiscover", "-r", ip_range]
        ...
        def run():
            ...
            try:
                ...
                self.log("netdiscover not found. Install it and run with sudo if needed.")

        threading.Thread(target=run, daemon=True).start()

    def stop_netdiscover(self):
        if self.netdiscover_process:
            self.netdiscover_process.terminate()  # graceful stop[web:28][web:43]
            self.netdiscover_process = None
