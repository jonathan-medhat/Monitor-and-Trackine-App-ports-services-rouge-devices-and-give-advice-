...
class CyberMonitor:
    ...
    def start_netdiscover(self):
        ...
        def run():
            ...
            try:
                ...
                self.log("netdiscover not installed")
    ...
    def generate_advice(self):
        ...
        for line in self.nmap_log:
            ...
            if "ssh" in l:
                advice.add("🔐 SSH found → Use key-based authentication.")
            if "http" in l:
                advice.add("🌐 HTTP detected → Switch to HTTPS.")

        if self.packet_count > 100:
            advice.add("📡 High traffic detected → Possible flood attack.")

        for line in self.traffic_log:
            if "icmp" in line.lower():
                advice.add("📶 ICMP traffic → Limit ping rate.")
            if "arp" in line.lower():
                advice.add("🧠 ARP packets → Watch for ARP spoofing.")

        if len(self.netdiscover_log) > 10:
            advice.add("🖧 Many devices detected → Check for rogue devices.")

        if not advice:
            advice.add("✅ System looks normal. No major threats detected.")
